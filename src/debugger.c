...
static const SDL_Color COLOR_BG       = {0x1A, 0x1A, 0x2E, 0xFF};
static const SDL_Color COLOR_PANEL    = {0x16, 0x21, 0x3E, 0xFF};
static const SDL_Color COLOR_TEXT     = {0xE0, 0xE0, 0xE0, 0xFF};
static const SDL_Color COLOR_LABEL    = {0x80, 0x80, 0x80, 0xFF};
static const SDL_Color COLOR_VALUE    = {0x00, 0xFF, 0x80, 0xFF};
static const SDL_Color COLOR_PC       = {0xFF, 0xFF, 0x00, 0xFF};
static const SDL_Color COLOR_FLAG_ON  = {0x00, 0xFF, 0x00, 0xFF};
static const SDL_Color COLOR_FLAG_OFF = {0x60, 0x60, 0x60, 0xFF};
static const SDL_Color COLOR_BUTTON   = {0x30, 0x50, 0x80, 0xFF};
static const SDL_Color COLOR_BUTTON_H = {0x40, 0x70, 0xB0, 0xFF};
static const SDL_Color COLOR_PAUSED   = {0xFF, 0x80, 0x00, 0xFF};
static const SDL_Color COLOR_RUNNING  = {0x00, 0xFF, 0x00, 0xFF};
static const SDL_Color COLOR_ADDR     = {0x80, 0xC0, 0xFF, 0xFF};
static const SDL_Color COLOR_HEX      = {0xFF, 0xC0, 0x80, 0xFF};
static const SDL_Color COLOR_ERROR    = {0xFF, 0x40, 0x40, 0xFF};
static const SDL_Color COLOR_HIGHLIGHT = {0xFF, 0xFF, 0x00, 0xFF};
...
static void update_pattern_texture(debugger_s *debugger_context) {
    ...
    if (SDL_LockTexture(debugger_context->pattern_texture, NULL, &pixels, &pitch) == 0) {
        ...
        uint32_t colors[4];
        colors[0] = NES_PALETTE[ppu->palette[0] & 0x3F];
        for (int i = 1; i < 4; i++) {
        ...
        switch (event->key.keysym.sym) {
            ...
            case SDLK_DOWN:
                ...
                if (debugger_context->debug_view_mode == DEBUG_VIEW_PPU) {
                    ...
                    if (debugger_context->oam_scroll_offset < 54) {
                        debugger_context->oam_scroll_offset++;
                    }
                } else {
...
static void draw_game_fullscreen(debugger_s *debugger_context) {
    ...
    int scale = DEBUGGER_WINDOW_HEIGHT / PPU_SCREEN_HEIGHT;
...
bool debugger_init(debugger_s *debugger_context, cpu_s *cpu, bus_s *bus) {
    ...
    debugger_context->run_speed = 100;
...
int main(int argc, char *argv[]) {
    ...
    bus_set_mirroring(bus, cart.mirroring);
    printf("Mirroring: %s\n",
           cart.mirroring == MIRROR_VERTICAL ? "Vertical" : "Horizontal");
